...
docker build --build-arg "GEMINI_VERSION=$gemini_version" -t repoarena/gemini:local -f "$repoarena_root/docker/agents/gemini.Dockerfile" "$repoarena_root"
docker build --build-arg "OPENCODE_VERSION=$opencode_version" -t repoarena/opencode:local -f "$repoarena_root/docker/agents/opencode.Dockerfile" "$repoarena_root"
